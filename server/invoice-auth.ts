...
const INVOICE_SECRET = process.env.INVOICE_SECRET || "bilyar-invoice-secret-change-in-production";
const LEGACY_INVOICE_SECRET = process.env.SESSION_SECRET;
...
export function signInvoiceId(orderId: number): string {
  return hmac(`invoice:${orderId}`, INVOICE_SECRET);
}
...
export function verifyInvoiceToken(orderId: number, token: string | undefined): boolean {
  ...
  const payload = `invoice:${orderId}`;
  const secrets = [INVOICE_SECRET, LEGACY_INVOICE_SECRET].filter(
...
export function createAdminSession(): string {
  ...
  return `${payload}.${hmac(payload, INVOICE_SECRET)}`;
}
...
export function verifyAdminSessionValue(value: string | undefined): boolean {
  ...
  const expected = hmac(payload, INVOICE_SECRET);
