...
Per [Deema API docs](https://docs.deema.me/reference/getting-started-with-deema-apis):

| Variable | Sandbox | Live |
|----------|---------|------|
| `DEEMA_BASE_URL` | `https://sandbox-api.deema.me` or `https://staging-api.deema.me` | `https://api.deema.me` |
| `DEEMA_API_KEY` | From [Merchant Portal](https://merchant.deema.me/) (Sandbox) | From Deema after go-live |
| `DEEMA_WEBHOOK_SECRET` | Same value as in Portal → [Webhook Settings](https://merchant.deema.me/webhook) | Same value as in Live Portal |

**Auth**: Deema uses `Authorization: Basic {API Key}`. BILYAR uses `DEEMA_AUTH=basic` (default). If auth fails, try `DEEMA_AUTH=basic64`.
...
**Sandbox verification**: Customer verification code in Sandbox for all phone numbers is `123456`.
...
1. Log in to the [Merchant Portal](https://merchant.deema.me/).
2. Open **Webhook** in the sidebar (or go to [Webhook Settings](https://merchant.deema.me/webhook)).
...
- [Deema API Reference](https://docs.deema.me/reference/getting-started-with-deema-apis)
- [Merchant Portal](https://merchant.deema.me/) – credentials, webhooks, order management
- [Webhook Settings](https://merchant.deema.me/webhook) – configure webhook URL and header secret
